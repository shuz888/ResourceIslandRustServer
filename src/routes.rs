...
pub async fn get_player_info(State(state): State<Arc<AppState>>, Path(player_name): Path<Option<String>>, Query(args): Query<HashMap<String, String>>) -> impl IntoResponse {
    let cfg = state.cfg.lock();
    let query_use_token = cfg.server.query_use_token.clone();
    let token_required = cfg.server.token.clone();
    drop(cfg);
    ...
    let player_name = if let None = player_name {
        args.get("player").cloned()
    } else {;
        player_name
    };
    if let None = player_name {
        return (StatusCode::BAD_REQUEST, Json(PlayerInfoResponse::with_error()));
    }
    let player = state.game_state.read();
    let player = match player.players.get(player_name.unwrap().as_str()){
        None => {
...
pub async fn ws_handler(State(state): State<Arc<AppState>>, Path(_player_name): Path<String>, ws: WebSocketUpgrade, Query(args): Query<HashMap<String, String>>) -> impl IntoResponse {
